...
(*     db/baseSwapStore.ts        transition() / patch() / fail(): one     *)
(*                                implementation, inherited by all four    *)
(*     db/swaps.ts                the send row, its SHAPE and migrations   *)
(*     db/receiveSwaps.ts         ditto, receive                           *)
(*     db/onchainSwaps.ts         ditto, onchain send                      *)
(*     db/onchainReceiveSwaps.ts  ditto, onchain receive                   *)
(*     db/driver.ts               the SqlDriver seam (WAL, synchronous)    *)
(*     send/orchestrator.ts       tick() / tickAll() / driveRows()         *)
(*   packages/solver-app/src/                                              *)
(*     worker.ts                  the queue fan-out and its safety claim   *)
(*                                                                         *)
(* THE CLAIM UNDER TEST                                                    *)
(*                                                                         *)
(* The module header of packages/solver-app/src/worker.ts asserts that     *)
(* money-safety does NOT depend on the in-process `inFlight` Set — which   *)
(* packages/solver-corridors/src/send/orchestrator.ts declares and tick()  *)
(* consults — but rests on the store's compare-and-swap alone.  A Go       *)
(* rewrite with N goroutines across M processes deletes `inFlight`.        *)
(* Everything in this module is arranged so that assertion can be          *)
...
(* refundLocktimeFor (packages/solver-core/src/core/send.ts) carries the   *)
(* `unilateralBound` term.  Permanent censorship now has a                 *)
(* server-independent escape in                                            *)
(* packages/solver-arkade/src/arkade/unilateralExit.ts, whose              *)
(* startUnilateralExit builds and drives that package.  It is              *)
(* operator-driven, outside the tick loop this module models, and has no   *)
(* LEGAL_EDGES entry to record it with (issue #184).  A server             *)
(* that flaps and returns exactly inside the contested window is still     *)
(* addressed by no automatic guard; modelling it would only restate what   *)
...
(***************************************************************************)
(* EXPOSURE ACCOUNTING.  committedSats() sums amount_sats over the shape's *)
(* `live` states — deliberately wider than `exposed`, because a quoted     *)
(* swap is capacity the provider may still have to honour.  Both lists and *)
(* the sum are in packages/solver-corridors/src/db/baseSwapStore.ts.       *)
(***************************************************************************)
