...
(*     arkade/reservations.ts       the in-process coin reservation        *)
(*                                  ledger                                 *)
...
(*       violates NoNetLoss at depth 29.  See the report.                  *)
(*                                                                         *)
...
(* with its own .cfg.  All seven produce a counterexample, and two ship a  *)
...
(*   OnchainReceive_UnconfirmedSettle.cfg                                  *)
(*                                   SettleNeedsConfirmation = FALSE       *)
(*                                   -> NoSilentLoss violated, 2s          *)
(*                                   (260,062 generated / 42,018 distinct, *)
(*                                   depth 19, measured on this model at   *)
(*                                   8367b70 with TLC 2.19)                *)
...
(*   OnchainReceive_MempoolRace.cfg  MempoolExclusive = FALSE              *)
(*                                   -> NoNetLoss violated, depth 29,      *)
(*                                   1,083,696 states / 160,264 distinct.  *)
(*                                   ADDED BY AUDIT: (A3) was the only     *)
(*                                   mutation knob here that no cfg ever   *)
(*                                   flipped, while OnchainSend has covered*)
(*                                   its own mirror of it all along.  The  *)
(*                                   client's L1 refund confirms over the  *)
(*                                   solver's already-broadcast claim after*)
(*                                   the Arkade side was taken, so the     *)
(*                                   client collects both legs.  An HONEST *)
(*                                   RESIDUAL like the send leg's, not a   *)
(*                                   regression: the shipped defence is the*)
(*                                   L1 claim WINDOW, not a state-machine  *)
(*                                   guard, and that window is what        *)
(*                                   intent-solver#160 is about.           *)
(*                                                                         *)
(* AN EIGHTH, ADDED BY AUDIT, because the seven above all leave the         *)
