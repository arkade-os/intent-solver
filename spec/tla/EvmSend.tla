...
(*     evm/erc20Token.ts          the approve/reset-approve dance          *)
(*                                                                         *)
...
(* evmTimeout is stored as a height via blocksForDuration, which floors    *)
(* the duration at the SLOWEST cadence and clamps to one block.  Fewer     *)
(* blocks means the solver's own refund opens no LATER in wall time than   *)
(* intended - blockTime.ts's stated safe direction for setting our own     *)
(* timelock.  This module models height as a second clock advancing        *)
(* inside a cadence band:                                                  *)
(*                                                                         *)
...
(*       (plan, case claiming).  See THE PARKED STATES below.              *)
...
(*       chain tip shows.  A pending lock tx can land AFTER the row went   *)
(*       terminal, putting the solver's ERC20 out with the books closed.   *)
(*       Green assumes lock fate is settled before the timeout; the        *)
(*       mutation lets it land late and NoNetLoss fails.  Finding F4,      *)
(*       STILL OPEN - and see (A8), which shipped the half of it the code  *)
(*       can actually close.                                               *)
(*                                                                         *)
...
(*                                                                         *)
(*       THE BOOKS ARE NOW READ AFTER THEY CLOSE, and that is deliberately *)
(*       NOT a constant here.  The sweep keeps asking the contract about a *)
(*       CLOSED row that reached a lock call until its wall refundLocktime *)
(*       (evmOrchestrator.ts watchLateLocks, over evmSendSwaps.ts          *)
(*       findClosedOverLock): a lock that lands late is reported, and a    *)
(*       claim of that lock puts the client's preimage back on the row.    *)
(*       It changes what the solver NOTICES, never what it broadcasts, so  *)
(*       not one guard above moved and EvmSend_LateLock.cfg still fires    *)
(*       NoNetLoss at the same witness.  Turning the notice into an        *)
(*       invariant would need an OPERATOR to act on it, and this module    *)
(*       has none - so it says that rather than adding a variable no       *)
(*       invariant here could read.                                        *)
(*                                                                         *)
(*       `refunded` IS NOW WATCHED TOO, and the reason it need not have    *)
(*       been is now EVIDENCE rather than inference.  (A4) writes that     *)
(*       word only for a refund PROVEN mined - a success receipt, or a     *)
(*       Refund event for THIS lock - which entails the ERC20 came back    *)
(*       only if the contract REVERTS a refund of a lock that is not       *)
(*       there.  That revert used to rest on knowing boltz-core; it is     *)
(*       now asserted against the deployed runtime bytecode on anvil       *)
(*       (test/e2e/evmErc20Swap.e2e.test.ts, "REVERTS a refund of a lock   *)
(*       that is not there"): status 0x0, no tokens moved, no Refund log.  *)
(*                                                                         *)
(*       THE WATCH IS THEREFORE REDUNDANT FOR THAT CONTRACT, and kept      *)
(*       anyway: the entailment belongs to whichever ERC20Swap a           *)
(*       deployment points a row at (evm_contract_address is per-row),     *)
(*       not to this module.  It costs one SQL token and REPORTS only -    *)
...
(*     in a mempool (A6).  The row's ENTRY into locking_evm is the filter, *)
(*     not evm_lock_txid: that column is patched after the broadcast, so a *)
(*     crash between them leaves it null over exactly the lock that must   *)
(*     not be skipped.                                                     *)
(*                                                                         *)
...
(*                                                                         *)
(* RE-RUN AT -workers 16 ON TLC 2.19 (rev 5a47802), and this block's       *)
(* figure is the one that reproduces: 22,039,291 generated, 2,841,354      *)
(* distinct, depth 48, 15min 28s, no error.  EvmSend.cfg's 21,513,363      *)
(* did not reproduce.                                                      *)
(*                                                                         *)
(* That does NOT make either a transcription error.  Both recorded runs    *)
(* say -workers 4, so worker count alone does not explain the gap, and     *)
(* GENERATED is evidently not stable across executions.  DISTINCT and      *)
(* DEPTH reproduced on every run of THIS GREEN CFG.  Compare on those;     *)
(* treat generated as indicative unless workers and TLC version are given. *)
(*                                                                         *)
(* THAT DOES NOT CARRY TO THE MUTATION CFGS.  A green run exhausts the     *)
(* state space, so distinct is deterministic.  A cfg that halts at the     *)
(* first violation stops when any worker finds one, so distinct is a       *)
(* function of thread scheduling: LightningSend_Broken.cfg at -workers 16  *)
(* gave 19,881 / 18,031 / 18,798 over three runs, all at depth 17.  On a   *)
(* violating cfg the stable facts are the INVARIANT NAME and the DEPTH.    *)
(*                                                                         *)
(* TLC DEFAULTS TO ONE WORKER, and run that way this cfg does not finish   *)
(* in 25 minutes - which reads as an intractable model rather than an      *)
(* unset flag.  Three attempts were lost to that before the re-run above.  *)
(*                                                                         *)
(* FINDINGS                                                                *)
(*                                                                         *)
...
(*                                                                         *)
(*     AND WHAT THE WATCH TOOK OFF IT.  The terminal row that closed over  *)
(*     a lock is no longer unobserved: a `stuck` or `refunded` row that    *)
(*     reached a lock call is re-read until its wall refundLocktime, so a  *)
(*     late lock is an operator event and a claim of it lands the preimage *)
(*     back on the row.  What that converts is the SILENT half - the       *)
(*     tokens leaving with nobody looking.  NoNetLoss is UNMOVED and this  *)
(*     cfg still fires it: noticing is not recovering, and the operator    *)
(*     who acts on the notice is outside this module.  See (A6).           *)
