...
(*       `stuck` row that reached a lock call until its wall               *)
(*       refundLocktime (evmOrchestrator.ts watchLateLocks, over           *)
(*       evmSendSwaps.ts findStuckOverLock): a lock that lands late is     *)
(*       reported, and a claim of that lock puts the client's preimage     *)
(*       back on the row.  It changes what the solver NOTICES, never what  *)
(*       it broadcasts, so not one guard above moved and                   *)
(*       EvmSend_LateLock.cfg still fires NoNetLoss at the same witness.   *)
(*       Turning the notice into an invariant would need an OPERATOR to    *)
(*       act on it, and this module has none - so it says that rather      *)
(*       than adding a variable no invariant here could read.              *)
(*                                                                         *)
(*       `refunded` IS NOT WATCHED, and does not need to be.  (A4) writes  *)
(*       that word only for a refund PROVEN mined - a success receipt, or  *)
(*       a Refund event for THIS lock - and the contract reverts a refund  *)
(*       of a lock that is not there (refundSweep.ts's `already_settled`   *)
(*       deferral is that revert, avoided).  So the word already entails   *)
(*       the ERC20 came back, and NoDoubleLock bounds the row to one lock  *)
(*       call, so none can land under it afterwards.  `stuck` is the only  *)
(*       terminal state reachable over a live or still-pending lock.       *)
(*                                                                         *)
...
(*  6. TERMINAL IS NOT UNOBSERVED.  A `stuck` row that once entered        *)
(*     locking_evm keeps being read against the contract until its wall    *)
(*     refundLocktime, because the lock it closed over may still be in a   *)
(*     mempool (A6).  The row's ENTRY into locking_evm is the filter, not  *)
(*     evm_lock_txid: that column is patched after the broadcast, so a     *)
...
(*     a lock is no longer unobserved: a `stuck` row that reached a lock   *)
(*     call is re-read until its wall refundLocktime, so a late lock is an *)
(*     operator event and a claim of it lands the preimage back on the     *)
(*     row.  What that converts is the SILENT half - the tokens leaving    *)
(*     with nobody looking.  NoNetLoss is UNMOVED and this cfg still       *)
(*     fires it: noticing is not recovering, and the operator who acts on  *)
(*     the notice is outside this module.  See (A6).                       *)
