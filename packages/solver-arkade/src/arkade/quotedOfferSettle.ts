...
export interface QuotedOfferIntent {
  ...
  carrierTerms?: { mode: 'purchase' | 'recycle' } | null
}
...
export const quotedOfferSettleFor = (deps: QuotedOfferSettleDeps): ((intent: QuotedOfferIntent) => Promise<string>) => {
  ...
  return async (intent) => {
    if (intent.carrierTerms?.mode === 'recycle') {
      throw new Error('recycle settlement requires the dedicated receive-carrier adapter')
    }
    ...
    return fulfill(
      ...
      },
      {
        ...
        // trusted, because the solver COMPILES this packet from terms it named
        // itself, and a truncated value describes an input that does not exist.
