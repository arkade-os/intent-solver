...
const fakeServices = (over: Record<string, unknown> = {}) => {
  ...
  return {
    ...
    tickErrors: { failing: [] },
    config,
    // The BOOT snapshots the overview's restart alert diffs the store against.
    // Same values here, so the quiet case is the default and a test that wants
    // drift says so. @see test/admin/configDrift.test.ts
    policy: config,
    assetMarkets: [],
