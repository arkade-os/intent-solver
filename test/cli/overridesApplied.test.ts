...
describe('createServices resolves stored overrides', () => {
  ...
  it('still exposes the ENVIRONMENT config, which the corridor-enable rules depend on', () => {
    ...
    const body = createServices()
    expect(body).toMatch(/const services: Services = \{\s*config,\s*policy,\s*bootPolicy: policy,/)
  })
})
...
describe('what must agree with the quoted terms reads policy, not env', () => {
  ...
  it('the asset markets, through the same resolver the console uses', () => {
    ...
    expect(body).toContain('assetCardMarketsFromPolicy({')
    expect(body).toContain('rfqMarkets: assetRfqMarketsFrom(assetMarkets,')
