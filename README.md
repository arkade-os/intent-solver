...
| `EVM_LOG_SCAN_RANGE`            | **default `10000`** — blocks per `eth_getLogs` request. The claim and refund scans page from a floor derived from the lock up to the tip, and 10k is the cap Alchemy and Infura publish. Set it to whatever your endpoint allows: a request over the cap is REJECTED, and the send corridor cannot tell a rejected claim scan from a client that has not claimed. **Setting it too HIGH is silent** and reproduces the exact failure the knob exists to prevent — every page exceeds the cap, every scan is rejected, and the solver never learns a preimage the client has already used. Too low only costs extra round trips; when in doubt, go lower |
