...
| `APPROVAL_THRESHOLD_SATS`                     | unset (no gate)                                          | sats at or above which a **send** leg holds and waits for a human before paying out. Unset means no gate and today's behaviour exactly; an explicit `0` gates every swap. Approve from the console with `approve-swap`; a swap nobody answers about refuses itself at its own deadline and the client's lockup goes to the refund sweep. Send legs only — on a receive leg the counterparty has already committed, so holding would freeze someone else's money                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID`     | unset                                                    | where business events and approval requests go. **Both** are needed — a token alone cannot address a message. **A credential**: env-only, never returned by the admin API, never logged, and withheld from every error message this service raises                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `SLACK_WEBHOOK_URL`                           | unset                                                    | the same events, to Slack. **The whole URL is the credential**, path included, and is treated exactly like the Telegram token above. Configuring neither transport leaves the solver making no notification calls at all                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
