...
| `ASSET_CARRIER_PRICING` | `true` nets the dust carrier an Arkade asset rides on out of whichever side of an RFQ quote supplies it — returned inside `to_amount` when the client fronted it, charged against the input when this solver delivers it — and publishes it as the quote's `carrier_sats`. Default `false`, so a running deployment's quoted prices do not move on upgrade. On, realised margin equals the advertised fee on both legs instead of the two cross-subsidising each other; the sats-leg dust floor applies either way, being a chain rule rather than a price |
| `OFFER_CHARGE_CARRIER`  | `true` charges a maker for the dust carrier this solver delivers on an asset-want offer. Default `false`, deliberately: on this path the MAKER prices the offer, and the registry card has no field that could tell them the carrier exists — so charging refuses an already-funded offer with no channel to say why. Off funds that carrier out of margin instead |
