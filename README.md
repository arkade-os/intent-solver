...
| `<CORRIDOR>_ENABLED`                          | `true`                                                   | whether this corridor QUOTES. `false` darkens it: its service is never constructed, and its pair is refused `unsupported_pair` at both ingresses (by name, before any quote exists) instead of being quoted and failed per swap. Rows already on disk stay readable and answerable, but the SWEEP STOPS: the watch loop ticks only the enabled set, so a darkened corridor's non-terminal rows no longer advance on their own. The one-shot commands still unwind them — `refund`, `onchain-refund-now` and `reclaim-l1-htlc` all build with `allCorridors` — but somebody has to RUN one, and the EVM receive leg has no unattended refund at all. **Drain a corridor before darkening it**; the runbook entry "When the solver refuses to boot on an _ENABLED value" has the procedure. Only the exact strings `true`/`false`; `FALSE`, `0` and `no` all throw, because a typo silently meaning "on" would leave a corridor quoting that an operator believes is dark                                                                                                                                                                                                                                                         |
...
| `ONCHAIN_RECEIVE_MAX_BAND_SATS`               | the corridor's own range                                 | how far a funded onchain-receive amount may sit from the amount it was quoted at, when the client asks for that flexibility by sending `min_from_amount`/`max_from_amount` on the quote request. Exists because an exchange withdrawal deducts its own fee, so what arrives is not what was quoted, and the client is offline by the time anyone finds out — the band is consent given BEFORE funding. **Narrows only**, and independent of `<CORRIDOR>_MAX_SATS` on purpose: capping how far an amount may DRIFT is a different risk from capping the amount, and an operator may want the first tight while the second stays wide. `0` is exactly the strict equality this corridor has always enforced. A client that sends neither field is unaffected whatever this is set to. **Narrowing this also turns off overfund tolerance by default** — the width that survives is placed entirely below the quote, and `ONCHAIN_RECEIVE_BAND_BELOW_SHARE` is how you get some back. Deliberate (an operator narrowing this is signalling float caution, and an overfund is what costs float under delta-preserving re-pricing) and no stricter on that side than strict equality already was, but not obvious from this knob's name. Non-integer or negative throws                                                                                   |
| `ONCHAIN_RECEIVE_BAND_BELOW_SHARE`            | `1` (all of it below)                                    | where a NARROWED tolerance band sits relative to the quote, as a fraction in `[0, 1]` of the width that survives `ONCHAIN_RECEIVE_MAX_BAND_SATS`. Only reachable when that cap actually binds — an operator who never narrows never reaches this. Defaults to keeping the whole retained width on the **underfund** side, because under delta-preserving re-pricing an underfund funds a smaller lockup than was already reserved and so cannot cost the operator, while an overfund consumes float and exposure headroom it never set aside. That is a policy default rather than a correctness property, which is why it is a knob: `0.5` splits it evenly, `0` offers the whole tolerance on the overfund side. Outside `[0, 1]` or non-finite throws                                            |
...
| `TAXI_URL`                                    | none                                                     | base URL of the Taxi operator the asset-RFQ **receive-carrier** rail recycles a client's dust carrier through. Unset = a `recycle` quote is refused by name rather than priced; a Taxi the payer names itself (`recycle_receiver`) is quoted and settled either way. The ONLY knob the rail adds — the Arkade Service key, the emulator key, the dust split, the address prefix and the locktime domain all come from the context this process already trusts, so naming an operator here cannot widen what it may charge or which coins it may admit. **On `bitcoin` this must be `https://`** unless the host is loopback, and `loadConfig` throws otherwise. A block-typed deployment additionally needs `CHAIN_TIP_ESPLORA_URL` (or `LND_ESPLORA_URL`): the minimum input expiry this solver will accept is anchored on the tip, and startup refuses rather than accept an unanchored floor |
| `TAXI_RECEIVER_ALLOW_PRIVATE`                 | `0`                                                      | `1` lifts the loopback/private-suffix refusal in the SSRF guard a **payer-named** Taxi URL is checked against (Ruling 3, `ops/taxiUrlGuard.ts`) — the regtest stack's own knob. Never affects `TAXI_URL` above, which keeps its own, looser, config-time gate |
...
| `EVM_QUOTE_VALIDITY_SECONDS`    | **default `60`**, integer in `[10, 900]` — defaulted because the value is policy, not chain fact: rfq-protocol.md §5 puts cross-asset quote windows "on the order of ~30 seconds". The quote's `valid_until` is `now + this`, snapshotted per row; funding first observed past it is refused and refunded, never filled at a stale rate                                                                  |
| `EVM_LOG_SCAN_RANGE`            | **default `10000`** — blocks per `eth_getLogs` request. The claim and refund scans page from a floor derived from the lock up to the tip, and 10k is the cap Alchemy and Infura publish. Set it to whatever your endpoint allows: a request over the cap is REJECTED, and the send corridor cannot tell a rejected claim scan from a client that has not claimed. **Setting it too HIGH is silent, and hides where it hurts.** A request spans `min(this, tip - floor + 1)`, so a recent swap is one short page that fits whatever you set and shows nothing wrong. It bites only once a scan is long enough to use the full span — the genesis fallback, and rows whose lock is far behind the tip — where the page is rejected, the rejection reads as "not claimed yet", and the solver never learns a preimage the client has already used. That is exactly the stuck old rows the scan exists for. Too low only costs extra round trips; when in doubt, go lower |
...
| `OFFER_CHARGE_CARRIER` | `true` charges a maker for the dust carrier this solver delivers on an asset-want offer. Default `false`: on this path the MAKER prices the offer from the card, so charging without the card saying so refuses an already-funded offer with no channel to say why. Off funds that carrier out of margin instead |
...
| `ASSET_CARRIER_PRICING` | `true` nets the dust carrier an Arkade asset rides on out of whichever side of an RFQ quote supplies it — returned inside `to_amount` when the client fronted it, charged against the input when this solver delivers it — and publishes it as the quote's `carrier_sats`. Default `false`, so a running deployment's quoted prices do not move on upgrade |
...
| `LND_ESPLORA_URL`                    | none (optional) | **required by BOTH onchain corridors**, though it read as receive-only for a long time and the send leg has in fact needed it since the spend lookup moved off lnd's own subscription. Three things lnd cannot answer: a client's funding transaction (wallet-scoped chain view, no per-output values — `findOutputs`); who spent an outpoint (it dispatches spends only on confirmation, and never for a third-party output — `findSpendWitness`, which the SEND leg calls from `whenAwaitingClaim`, `whenRefundingOnchain` and `reclaimOnchainHtlc`); and whether a transaction we broadcast landed (`transactionOutcome`). Each fails loudly by name rather than under-reporting, so unset is a corridor that cannot advance rather than one that answers wrongly |
...
| `WATCH_SYNC_MS`                                                | 500 ms                                            | `packages/solver-app/src/cli.ts` — puts a new swap's script on the subscription before the funding arrives, not a sweep behind it                                                                                                                                      |
| `FULL_SWEEP_MS`                                                | 3000 ms                                           | `packages/solver-app/src/cli.ts` — the only pass that advances a row waiting on a deadline, the EVM legs' only driver, and inside the SDK's failsafe poll                                                                                                               |
